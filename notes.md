...
Fortunatly cloud haskell already has a solution for this kind of problem. The *distributed-static* library provides means to serialze functions as long as they are known at compile time. The problem is that ghc currently doesn't expose a way to discover if something is defined at compile time. This way *distributed-static* has to manually manage these definitions. This mechanism doesn't support classes well though. There is no way of discovering every implementation of a class. To get around this we need another class that provides a *static* way to serialize a conversion function. Additionally using this serialization mechanism somewhat locks into the cloud haskell ecosystem though. *dakka* itself would have to depend on cloud haskell packages.
