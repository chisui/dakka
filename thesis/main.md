...
It doesn't make creating distributed systems in Haskell easy but is capable of performing the heavy lifting.

Unfortunately Cloud Haskell has to be somewhat opinionated since some features it provides wouldn't be possible otherwise. 
...
Haskell itself doesn't provide a mechanism to promote functions to type families. 
...
Type families allow Non-Linear Patterns, that is the same variable may occure multiple times inside of the pattern, but type classes don't. 
...
Unfortunately it isn't possble to use the `forall` keyword in the context of instance declrations (yet [@hku-qantified-clas-constraints]). 
...
In the process of compiling Haskell, all type information is removed since it isn't needed at runtime. 
Type information may be useful at runtime sometimes. If a type is hidden via existential quantification it may be useful to be able to get a `String` representation of the captured type for debug and/or `Show` purposes for example. 
...
The `Show` implementation of `TypeRep` doesn't produce output that is equivalent to the way types are represented in Haskell error messages. 
...
Unfortunately we can't derive `Generic` for data types that use existential quantification and thus can't get a `Binary` instance for free.
...
Notice that the resulting `ActorContext` doesn't have `a` as its Actor type but rather some other type `b`. 
...
References of this kind can't be be created by the user since you shouldn't be able to associate any `ActorId` with any Actor type, since there is no way of verifying at compile time that a given id is associated a given Actor type. 
...
So if we don't override the `Creates` type family for a specific Actor, we assume that this Actor does not create any other Actors.
...
The `Creates` typefamily is incredibly useful for defining assumtions that concern the hierarchy of the Actorsystem.
